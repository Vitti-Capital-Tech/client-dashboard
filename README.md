...
- **Auth & session:** **real Supabase Auth**, with two doors and two credentials. Clients sign in at `/login` with **a password or a one-time code**; staff sign in at `/staff/login` with **a code only** — see §4.4 for why staff never hold a password. New clients register themselves at `/signup` (3 steps, §4.4a). The root `proxy.ts` refreshes the session cookie each request and redirects both ways (unauthenticated → `/login`, authenticated → their portal); `lib/session.ts` reads identity via `supabase.auth.getUser()` (`getActiveClientId` / `getActor`), with the workspace role in `app_metadata.role`, stamped from the email domain by a trigger on `auth.users`. Only `vitti_view` (staff's inspected client) and `vitti_account` (the client's active account) remain cookies. Route protection and RLS are both live; real **TOTP MFA** is still the open item.
