...
│   ├── tracker-style-plan.mjs  # Scans Template's formatting (~8.5 min, 1207 Graph reads) and
│   │                           #   stores it, because the ingest routes have 60s and the
...
| Template's look, scanned once | ✅ `…_placement_style_plan.sql` — `placement_style_plans`, one `jsonb` row per workbook. Recovering Template's formatting costs **504,324 ms and 1,207 Graph format reads**; applying it costs **73 writes**. Every ingest route has 60 seconds, so a live scan could only ever finish on an instance that had already paid for it — and it showed: `IPT` and `IPT (b)` carry identical column widths, all sixteen ~20pt narrower than Template's, replayed from a stale in-process plan after Template was widened, with nothing recording which Template it came from. There is no shortcut in the API — both `worksheets/copy` and `range/copyFrom` were probed on this tenant in v1.0 and beta and neither exists — so the reconstruction stays and only its cadence changed. `npm run tracker:plan` scans it out-of-band with a generous budget; `shape` and `scanned_at` ride along so drift, age and a truncated scan each report themselves. An absent plan falls back to scanning; an absent table names its migration. LLD §8.42 |
