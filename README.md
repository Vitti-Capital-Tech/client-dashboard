...
> **Watch the clock on Hobby.** The route is capped at 60s there, and a cold run pays ~17s to download and parse the Placement Trackers before importing and recomputing. Nothing is corrupted if it is cut off — the importers are idempotent and the watermark only advances on a clean run — but it will not finish either, and the symptom is silence: `cron.job_run_details` shows a successful POST while `ingest_runs` stays empty. If that happens, cache the parsed trackers in Postgres so the recompute reads them instead of re-parsing unchanged workbooks daily.
