...
export default function ClientDashboard() {
  ...
  const options = clientOptions(db, clientId);

  const pv = portfolioValue(db, clientId);
  const dpl = dailyPL(db, clientId);
  const tpl = totalPL(db, clientId);
  const mtd = pv * 0.018;
  const ytd = pv * 0.064;

  const liveDeal = db.placements.find(p => p.stage === "open");
  const myBid = liveDeal ? liveDeal.bids.find(b => b.c === clientId) : null;

  // Filter alerts visible to this client
  const clientAlerts = db.alerts.filter(a => a.client === clientId && !a.ack).slice(0, 3);
  ...
  const renderTicker = () => {
    const tickerItems = db.indices.map(x => {
      const isUp = x.chg >= 0;
  ...
  const getSuggestions = () => {
    ...
    const trimHolding = positions.find(p => {
      const sig = db.signals[p.code];
      return sig && (sig.action === "Take profit" || sig.action === "Trim");
    });
    if (trimHolding) {
      const sig = db.signals[trimHolding.code];
      list.push({
        tone: "amber",
    ...
    <div className="space-y-4">
      ...
      <div className="card bg-navy text-[#dfe2ee] border-navy p-5 rounded-[14px] shadow-shadow space-y-3 relative overflow-hidden">
        ...
        <div className="flex justify-between items-center text-xs">
          ...
          <span className="text-mut-d font-medium">{db.note.time} &middot; auto-generated</span>
        </div>
          ...
          <div className="card bg-white border border-line rounded-[14px] p-4.5 shadow-shadow space-y-3">
            <div className="flex justify-between items-center text-xs">
              ...
              <button 
                ...
                className="text-green-d font-semibold text-xs underline underline-offset-2 hover:opacity-85 cursor-pointer"
              >
                {db.alerts.filter(a => a.client === clientId && !a.ack).length} unread
              </button>
            </div>
            <div className="space-y-2">
