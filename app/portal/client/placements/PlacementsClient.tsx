...
export function PlacementsClient({
  ...
  const handleOpenDeal = (id: string) => {
    ...
    const bid = p.bids.find(b => b.clientId === clientId);
    if (bid) {
  ...
  const handlePlaceBidReview = () => {
    const p = placements.find(x => x.id === selectedDealId);
    if (!p) return;
  ...
  const handleConfirmBid = async () => {
    ...
    await placeBid(selectedDealId, amt);
  ...
  const handleWithdrawBid = async () => {
    ...
    if (confirm(`Are you sure you want to withdraw your bid on ${p.name}?`)) {
      await withdrawBid(selectedDealId);
      setSelectedDealId(null);
      alert("Bid withdrawn.");
    }
  };

  const handleBpayPayment = async () => {
    if (!selectedDealId) return;
    await notifyBpayPayment(selectedDealId);
                ...
                <tbody className="divide-y divide-[#f0ede5] font-medium">
                  <tr>
                    ...
                    <td className="py-2 text-right font-mono">
                      {p.closeDate ? new Date(p.closeDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "—"} &middot; 4:00pm
                    </td>
                  </tr>
                  <tr>
                    <td className="py-2 text-mut font-semibold">Allocations</td>
                    <td className="py-2 text-right font-mono">
                      {p.allocDate ? new Date(p.allocDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "—"}
                    </td>
                  </tr>
                  <tr>
                    <td className="py-2 text-mut font-semibold">Settlement</td>
                    <td className="py-2 text-right font-mono">
                      {p.settleDate ? new Date(p.settleDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "—"}
                    </td>
                  </tr>
                  <tr>
                    <td className="py-2 text-mut font-semibold">Allotment</td>
                    <td className="py-2 text-right font-mono">
                      {p.allotDate ? new Date(p.allotDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "—"}
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div className="card bg-white border border-line rounded-[14px] p-4.5 shadow-shadow h-fit space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="text-[11.5px] text-mut mt-1">
                ...
                {p.opts && p.opts !== "None" && " + free attaching options"}
              </div>
              {!isValid && amt > 0 && (
            ...
            <div className="space-y-1 pt-1.5 select-none">
              <div className="text-[11.5px] text-mut flex items-center gap-1.5">
                ...
                <span>s708 verified &middot; expires {s708}</span>
              </div>
  ...
  const renderTrackingTimeline = (p: PlacementRow, bid: BidRow) => {
            ...
            <div className="card bg-white border border-line rounded-[14px] p-5 shadow-shadow">
              <div className="space-y-0.5">
                ...
                {timelineRow(
                  ...
                  "Bids close",
                  `${p.closeDate ? new Date(p.closeDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "—"} &middot; 4:00pm${p.stage === "open" ? " · you can amend until then" : ""}`
                )}
                ...
                {timelineRow(
                  ...
                  isAllocated
                    ? `${p.allocDate ? new Date(p.allocDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "—"} &middot; ${bid.alloc === 0 ? "nil allocation" : (bid.alloc! < bid.amount ? `scaled to ${Math.round(bid.alloc! / bid.amount * 100)}%` : "filled in full")}`
                    : `Expected ${p.allocDate ? new Date(p.allocDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "—"}`
                )}
                    ...
                    <div className="body">
                      ...
                      <div className="s text-xs text-mut leading-normal">
                        {isPaid
                          ...
                          : `By ${p.settleDate ? new Date(p.settleDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "—"} &middot; $${bid.alloc!.toLocaleString("en-AU")}`}
                      </div>
                ...
                {bid.alloc! !== 0 && timelineRow(
                  isSettled ? "done" : "todo",
                  "Allotment & quotation",
                  `${p.allotDate ? new Date(p.allotDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "—"}${isSettled ? " · holding now in your portfolio" : " · holding appears in your portfolio"}`
                )}
              </div>
            </div>
          </div>
  ...
  if (selectedDealId) {
    const deal = placements.find(x => x.id === selectedDealId);
    if (deal) {
      const bid = deal.bids.find(b => b.clientId === clientId);
      if (bid) {
            ...
            return (
              ...
              >
                ...
                <div className="text-xs text-mut leading-normal select-none">
                  ...
                  {p.opts && p.opts !== "None" && ` &middot; ${p.opts}`}
                </div>

                <div className="font-mono text-[11px] text-mut select-none">
                  {p.stage === "open"
                    ? `Closes ${p.closeDate ? new Date(p.closeDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "—"}`
                    : `Opens ${p.closeDate ? new Date(p.closeDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "—"}`}
                </div>
