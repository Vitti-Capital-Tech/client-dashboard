...
export default function AskVittiPage() {
  ...
  const [messages, setMessages] = useState<Message[]>(() => [
    {
      role: "ai",
      text: `Good morning, ${client.name.split(" ")[0]}. I’m Vitti Intelligence. Here’s your briefing for Friday, 12 Jun 2026:`,
      brief: true
    },
    {
      role: "ai",
  ...
  const handleSendMessage = (text: string) => {
    ...
    if (isPortfolio) {
      ...
      const liveDeal = db.placements.find(p => p.code === "MRD");
      const hasBid = liveDeal?.bids.some(b => b.c === clientId);
      const myBid = liveDeal?.bids.find(b => b.c === clientId);
      const bidNote = myBid ? ` You have placed a bid of $${myBid.amount.toLocaleString("en-AU")}.` : "";
      ...
      const urgentOpts = clientOptions(db, clientId).filter(o => o.status === "open" && isITM(o) && o.dte <= 14);
