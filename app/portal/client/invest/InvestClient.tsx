...
export function InvestClient({
  ...
  const handleGeneratePlan = () => {
    ...
    const goalObj = GOALS.find(g => g.k === objective) || GOALS[0];
    // Filter ideas that match the themes of the chosen goal
    let pool = ideas.filter(i => goalObj.themes.includes(i.theme));
    if (pool.length < 3) pool = [...ideas];
    ...
    const deriveTF = (i: IdeaRow) => {
      if (i.placementId || (i.last !== null && i.entryLo !== null && i.entryHi !== null && i.last >= i.entryLo && i.last <= i.entryHi)) return "Tactical";
    ...
    const buckets: Record<string, IdeaRow[]> = { Tactical: [], Core: [], Strategic: [] };
    pool.forEach(i => {
  ...
  return (
    <div className="space-y-4">
      ...
      {plan ? (
        <div className="card bg-white border border-green rounded-[14px] p-5 shadow-shadow space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {plan.items.map(it => {
              const i = ideas.find(x => x.code === it.code);
              ...
              return (
                <div key={it.code} className="border border-line rounded-xl p-4.5 space-y-3">
                  ...
                  <div className="flex items-center gap-1.5 select-none">
                    ...
                    {isHeld && (
                      <span className="pill text-[10px] font-semibold px-2 py-0.5 rounded-full bg-green-bg text-green-d">held</span>
                    )}
                  </div>
      ...
      {showIdeaModal && (
        <div className="fixed inset-0 bg-navy/55 backdrop-blur-[2px] z-50 flex items-center justify-center p-4.5">
          {(() => {
            const i = ideas.find(x => x.code === showIdeaModal)!;
            const up = i.target && i.last ? Math.round((i.target / i.last - 1) * 100) : null;
            return (
