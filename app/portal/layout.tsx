...
export default function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  const client = db.clients[clientId] || { name: "James Halloran", av: "JH", type: "Individual" };

  const handleSignOut = () => {
    // Audit log will be generated during setRole inside context
    setRole("client");
    router.push("/");
  };
  ...
  const visibleAlerts = db.alerts.filter(a => role === "admin" || a.client === clientId);
  const unreadAlerts = visibleAlerts.filter(a => !a.ack);
  const alertsCount = unreadAlerts.length;

  // Pending Allocations count (admin only)
  let pendingAllocCount = 0;
  if (role === "admin") {
    db.placements.forEach(p => {
      if (p.stage === "closed") {
        p.bids.forEach(b => {
          if (b.alloc === null) pendingAllocCount++;
        });
      }
    });
  }
  ...
  // Web Layout (Responsive Layout by default)
  return (
