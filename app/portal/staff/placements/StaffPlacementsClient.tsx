...
export function StaffPlacementsClient({
  ...
  const handleOpenPlacement = (id: string) => {
    ...
    if (p) {
      ...
      const settleLabel = p.settleDate
        ? new Date(p.settleDate).toLocaleDateString("en-AU")
        : "—";
      setClientMessage(`Your allocation in ${p.name} (${p.code}) is confirmed. Payment details are in your portal; settlement is ${settleLabel}.`);
    }
  };
  ...
  const getScalingFactor = (p: PlacementRow, method: string) => {
  ...
  const handlePublishAllocations = async () => {
    ...
    p.bids.forEach(b => {
      allocations[b.clientId] = calculateAllottedAmount(b.amount, f);
    });

    // Persist allocations via the server action
    await scaleBids(selectedPlacementId, allocations);
  ...
  const handleSettleDeal = async () => {
    ...
    if (confirm(`Confirm settlement for ${p.code}? This will issue ordinary shares and attaching options to all allocated clients.`)) {
      await settlePlacement(selectedPlacementId);
                ...
                <table className="w-full border-collapse text-left text-xs font-medium">
                  ...
                  <tbody className="divide-y divide-[#f0ede5]">
                    {p.bids.length === 0 ? (
                      ...
                      p.bids.map(b => {
                        const cl = clientsById[b.clientId] || { name: b.clientId, initials: "???" };
                ...
                return (
                  <tr key={p.id} className="hover:bg-[#faf9f5]">
                    ...
                    <td className="px-4.5 py-3.5 text-mut font-mono text-[11px]">
                      {p.closeDate
                        ? new Date(p.closeDate).toLocaleDateString("en-AU", { day: "numeric", month: "short" })
                        : "—"}
                    </td>
