...
export default function StaffOptionsPage() {
  ...
  const [uoClient, setUoClient] = useState("C1");
  ...
  const activeOptions = db.options.filter(o => {
    if (o.status !== "open") return false;
    if (filterClient !== "all" && o.c !== filterClient) return false;
  ...
  const lapsedExpiredOptions = db.options.filter(o => {
    if (o.status === "open") return false;
    if (filterClient !== "all" && o.c !== filterClient) return false;
    return true;
  });
  ...
  const handleAddOptionSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const qty = parseInt(uoQty.replace(/[^0-9]/g, ""), 10) || 0;
    const strike = parseFloat(uoStrike) || 0;
    const under = parseFloat(uoUnder) || 0;
    const dte = parseInt(uoDte, 10) || 0;

    // Simulate addition (normally we would call a DB mutator, but since we are doing controlled mocks we can alert)
    alert(`Unlisted option ${uoCode.toUpperCase()} added on register for ${db.clients[uoClient]?.name || uoClient}.`);
        ...
        <div className="flex items-center gap-1.5">
          ...
          >
            ...
            {Object.keys(db.clients).map(cid => (
              <option key={cid} value={cid}>{db.clients[cid].name}</option>
            ))}
          </select>
        </div>
