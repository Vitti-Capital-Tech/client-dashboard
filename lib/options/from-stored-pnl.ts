...
/**
 * The narrower shape the client portal's Options tab renders.
 *
 * A client is shown their own holdings and nothing about how the desk arrived at
 * them — no cost basis, no pricing method, no account column, since every row
 * they can see is theirs. `itm` and `intrinsicValue` are lifted off the shared
 * moneyness verdict rather than recomputed, so the badge a client sees is the
 * same verdict the adviser sees.
 *
 * Declared here, beside the derivation, because the page that maps into it is a
 * Server Component and the component that renders it is a client one — a type
 * they can both import has to live outside both.
 */
export type ClientOptionView = {
  ...
  listed: boolean;
  qty: number;
  /** Null where the terms could not be read. Never render this as 0. */
  strike: number | null;
  under: number | null;
  dte: number | null;
  expiryDate: string | null;
  source: string | null;
  status: OptionTableItem["status"];
  itm: boolean;
  intrinsicValue: number;
};

export function toClientOptionViews(items: OptionTableItem[]): ClientOptionView[] {
  return items.map((i) => ({
    id: i.id,
    code: i.ticker,
    name: i.company,
    ...
    qty: i.quantity,
    strike: i.strike,
    under: i.underlyingPrice,
    dte: i.dte,
    expiryDate: i.expiryDate,
    source: i.source,
    status: i.status,
    itm: i.money.isItm,
    intrinsicValue: i.money.intrinsicValue,
  }));
}
