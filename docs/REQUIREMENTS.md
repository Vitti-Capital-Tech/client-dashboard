...
| F19 | **Option expiry on the client's Options tab** | ✅ Done | Expiry column (date plus time left, red inside 30 days) and an expiry filter (within 30 / 90 days / 12 months, later, expired, unknown). Dates come from the same `parseExpiry` the alert scanner uses, so the tab and the alerts agree. Staff alerts (drawer and page) also now show the client's full name instead of a ref or initials. See LLD §8.60 |
