...
- **Persistence is live on Supabase.** The production **PostgreSQL schema** ([`db/schema.sql`](../db/schema.sql))
  is applied as the first ordered migration (`supabase/migrations/`); there is no seed file, the data arrives through the broker import.
...
|---|------|--------|------------------------|
| F1 | **Auth & sessions** | ⏳ Partial | *Now:* **real Supabase Auth** on two doors — clients at `/login` by password **or** emailed code, staff at `/staff/login` by code only (staff hold no password, enforced in four places incl. a DB trigger). Client **self-registration** at `/signup`, **password reset** at `/reset-password`, and self-service login management at `/portal/client/settings` (password, login email, **additional logins for the same account** — see F15 — and sign out everywhere). Session refreshed by `proxy.ts`, identity via `getUser()`, role in `app_metadata.role` stamped from the email domain, **route protection** both ways, and **RLS** (see F2/§5). *Still needed:* **TOTP 2FA** — the emailed code is a real credential but an alternative to the password, not a second factor on top of one; that means Supabase MFA (`auth.mfa`) |
...
| F11 | **One login, several existing accounts** | ✅ Done | A client verifies an account number against the register (rate-limited, audited, name only — never the owner) and then claims it; staff verify against the broker record; approval re-parents the account and everything denormalised against it in one `SECURITY DEFINER` transaction. Now also the **last step of sign-up**, so a new login arrives with its first account attached. Approval has **two outcomes** since F15: an account whose owner cannot sign in is re-parented as before; one whose owner *can* is not moved at all — the claimant's login joins that owner instead, so both end up with access. `preview_account_claim` shows the desk which before they press the button. See LLD §8.34, §8.48 |
...
| F14 | **Client manages their own login** | ✅ Done | `/portal/client/settings` — change password (current one verified by using it, since `secure_password_change` is off), set a first password by emailed code for the broker-provisioned logins that have none, change the login email with confirmation from **both** addresses, and sign out of every device. Two triggers make the email change safe: one refuses moving a login onto the staff domain, the other keeps the client's login row in lockstep with `auth.users.email`. Notification preferences deliberately omitted — nothing in the app sends client email. See HLD §4.8, LLD §8.39 |
| F15 | **Several logins, one client** | ✅ Done | A couple, two SMSF trustees, a principal and their accountant — previously all answered with *share the password*. `client_emails` holds every address that can sign in as a client and is what `current_client_id()` and `lib/session.ts` resolve against; `clients.email` survives as a trigger-maintained mirror of the primary, because the staff register, the claim queue and the rail in `approve_account_claim` read it as a plain column. Adding one is sign-up in miniature — registered without a password, code emailed, the row that grants access written only after `verifyOtp` — and the verification runs on a deliberately cookie-less Supabase client, since on the ordinary one it would sign the caller out of the account they are editing. Removing one deletes its `auth.users` row, so revoked access is actually revoked. Closed a live hole on the way: service-role actions may not use `getActiveClientId()`, whose "first client" fallback is inert under RLS but is an account takeover when the writer bypasses it. Two doors reach this: the existing client adds an address from Settings, or a newcomer claims the account at sign-up and staff approve (F11). The audit log names the acting address once a client has more than one login. See HLD §4.9/§4.9a, LLD §8.46/§8.48 |
...
| F15 | **Theme & appearance customization** | ✅ Done | Reactive CSS variables runtime with 6 presets, custom hex pickers, real-time WCAG AA (4.5:1) validator, zero-flash synchronous script, and dark-mode table contrast rules (Historical P&L open/closed row separation, elimination of `text-navy` and `#faf9f5` hover traps). Accessible to clients at `/portal/client/customise` and staff advisers at `/portal/staff/customise` via sidebar and user menu. See HLD §3.1m, LLD §8.44 |
| F16 | **Private transactions** | ✅ Done | What a client holds via us that the broker does not custody — off-market parcels, transfers, unlisted assets — entered by the desk from the client's Portfolio tab and shown on the client's own screen badged **Private**. `is_private` rides on `trades`, `positions` and `pnl_summary` so these inherit every path the broker's rows already travel; the holdings-snapshot full replace is scoped to exclude them, or the 05:30 run would delete each one the morning after it was keyed. Unlisted assets carry a dated desk valuation (`manual_price`), ranked below any live market price. Entered one at a time, or **imported from a `.xlsx`/`.csv`** in the historical-trades shape — parsed in the browser, previewed row by row with every rejection named, written idempotently on `(reference, code, side)`, and recomputed once for the whole file rather than once per row. *Still needed:* an **edit/delete** surface for a private line already entered. See HLD §3.1d, LLD §8.56 |
| F17 | **Desk alerts on account requests** | ✅ Done | A client registering, claiming an extra account, or asking for a merge now emails the desk instead of waiting to be noticed on the Merge requests page. Sent through the **existing Microsoft Graph app registration** — no email provider, no second key, no new sending domain — and gated on `STAFF_NOTIFY_TO`, so an unconfigured deployment behaves exactly as before. Needs the `Mail.Send` **application** permission granted once in Azure; a 403 says so by name in the log. Sent from `after()` so a mail failure can never fail a client's request. *Still needed:* nothing for this; a client-facing confirmation email would be a separate decision. See README §4.2, LLD §8.57 |
| F18 | **Net P&L on the client's Home and Portfolio** | ✅ Done | One card that answers *what have I made in total*: all-time realised (the Historical tab's replay) plus unrealised on current holdings (the Holdings tab's footer, at last price, incl. unlisted grants), with both halves shown so the total can be checked against the page. Replaces the stored-total *Profit & loss* card, whose `realised + open` figure did not equal the two tabs added together. Follows the account filter; does not move with the Historical date picker. Home shows the same figure across all accounts, computed on the server; both screens build it through one module (`lib/pnl/net-pnl.ts`), and a test asserts they agree. See LLD §8.58 |
| F19 | **Option expiry on the client's Options views** | ✅ Done | On both the sidebar Options page and Portfolio's Options tab: expiry column (date plus time left, red inside 30 days) and an expiry filter (within 30 / 90 days / 12 months, later, expired, unknown). Dates come from the same `parseExpiry` the alert scanner uses, so the tab and the alerts agree. Staff alerts (drawer and page) also now show the client's full name instead of a ref or initials. See LLD §8.60 |
