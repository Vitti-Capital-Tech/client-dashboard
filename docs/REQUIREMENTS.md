...
|---|------|--------|------------------------|
| F1 | **Auth & sessions** | ⏳ Partial | *Now:* **real Supabase Auth** on two doors — clients at `/login` by password **or** emailed code, staff at `/staff/login` by code only (staff hold no password, enforced in four places incl. a DB trigger). Client **self-registration** at `/signup`, **password reset** at `/reset-password`, and self-service login management at `/portal/client/settings` (password, login email, sign out everywhere). Session refreshed by `proxy.ts`, identity via `getUser()`, role in `app_metadata.role` stamped from the email domain, **route protection** both ways, and **RLS** (see F2/§5). *Still needed:* **TOTP 2FA** — the emailed code is a real credential but an alternative to the password, not a second factor on top of one; that means Supabase MFA (`auth.mfa`) |
