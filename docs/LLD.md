...
- **A date range can only describe realised money.** Unrealised P&L is a cost base against *today's* price: it belongs to no date, and there is no price history in this app to value a holding as at an earlier one (the same reason the hardcoded growth curve was removed — §8.33). So `realizedBetween(sells, from, to)` answers "profit on what was **sold** between these dates", and the card says exactly that, twice. Captioning it as a period *return* would be the wrong number with no way for the client to tell.
