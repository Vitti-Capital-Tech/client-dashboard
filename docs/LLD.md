...
### 8.41 The client's Portfolio *is* the desk's tables (`lib/pnl/summary-rows.ts`, `app/components/PnlRow.tsx`, `clientSummary`)

§8.33 shared the *derivation* behind the Options tab and noted that sharing the columns is what stops the next divergence. This does that for the rest of the page: the client's Portfolio now carries the console's **Holdings · Historical P&L · Options** tabs, with Analytics unchanged.
...
- **The period picker moved onto the P&L-by-company table, because two tables of the same shape is not a feature.** The dated window used to be its own card, with its own contributors table listing holding, units sold, cost, proceeds and P&L — the same five things the table beneath it was already listing, leaving the reader to work out which one answered their question. There is one table now. **All time** is a distinct state (`range === null`), not the widest window, and it shows every parcel ever held, open ones included — the reference view. Narrow the period and the table switches to the sales inside it, built into the same `PnlSummaryRow` shape so one table body renders both. `buyQty` is left at zero in that mode on purpose: the column means "units bought" and a window has no answer for it, since the parcel being sold was acquired at some earlier date quite possibly outside the range.
- **The pills shrink with it.** Open, Matched, Unlisted Options and the rest describe the *state* of a position, and every row in a realised view is a completed sale — they would all read zero and invite a click that cannot work. A range leaves `All / Profit Only / Loss Only`. Changing the period resets the filter and the page, since a `Matched` pill carried into a realised view matches nothing and reads as an empty account.
...
- **Three tables were keyed on the ticker, which drops rows under All accounts.** A client holding EOS in two accounts has two EOS rows, and a duplicate React key silently renders one of them. Holdings, the P&L table and the Options register are all keyed by position in the list now.
