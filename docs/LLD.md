...
- **The migration backfills every existing row as written.** Without it the first run after deploy would decide ~200 candidates going back to January are owed tabs and start appending to the desk's live book. Apply it **before** the code that reads it: the reader degrades to a loud note, but a deal arriving between migration and deploy is backfilled as settled and needs a tab by hand.
