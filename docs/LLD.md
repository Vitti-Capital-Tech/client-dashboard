...
- Both are staff-only by RLS. One caveat worth knowing: `ingest_runs` and `ingest_attachments` are **absent from the generated `Database` types** (the file carries 29 tables and neither of that pair is among them), so the read casts through `any` with the row shape spelled out beside it. The real fix is `supabase gen types`.
