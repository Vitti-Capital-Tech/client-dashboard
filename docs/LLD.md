...
**Reads + triggers.** `lib/data/pnl.ts` (`getClientStoredPnl`, `getClientLatestPnlRuns`) keeps rows at account grain so the island applies the same account filter as everything else. `app/actions/pnl.ts` exposes **Recalculate** (one client) and **Rebuild all P&L** (every account, for the first fill or after an engine change); both are staff-gated *before* the service-role client is ever constructed, and both are audited. The tab shows a "calculated at" stamp and the run's warnings — a stored figure is only as good as its age.
...
- **The watermark advances only on a clean run.** A quarantined or failed file must be re-offered tomorrow, not skipped past. This is the one place where being *conservative* (re-reading mail) is free and being optimistic is not.
...
- **`/api/ingest/morning`** — `maxDuration = 300`, since the tracker parse alone can be ~48s before any import. A non-ok run returns **HTTP 500** so the platform's cron monitoring reports a failure rather than a quiet success with sad JSON inside. `POST` is aliased to `GET` for hand-triggered catch-up runs.
...
**The 60s ceiling is the thing to watch on Hobby.** A cold run pays ~17s to download and parse the Placement Trackers before importing and recomputing every account the snapshot touched. Nothing is corrupted if it is cut off — idempotent importers, and the watermark only advances on a clean run — but it will not finish, and the symptom is silence: `cron.job_run_details` shows a successful POST while `ingest_runs` stays empty. The fix is to stop re-parsing unchanged workbooks daily by caching the parsed trackers in Postgres.
