...
- **The Excel API edits in place; the file is never round-tripped.** The workbook is 13 MB and ~190 sheets of formulas, formatting and validation, and it is the desk's live book. Downloading it, editing with a spreadsheet library and uploading it back would rewrite all 190 sheets to change two, silently dropping every feature the library does not model. Graph copies `Template`, patches five cells and appends one row; nothing else is touched.
