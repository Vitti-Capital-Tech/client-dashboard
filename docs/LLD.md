...
- **Server Actions & Web Requests (`app/actions/pnl-calculator.ts`):**
  ...
  - `loadConfiguredPlacementTrackersAction()`: loads the standing `PLACEMENT_TRACKER_URL` link(s) so the desk never re-pastes them. Accepts several links separated by commas/semicolons/newlines (one workbook per year), reported individually so one dead link costs only itself.
    - **Sequential, not concurrent.** Measured on the real trackers (12.5 MB / 9.3 MB from SharePoint): download is 3s each, **parsing is 30s and 18s** — CPU-bound and single-threaded, so parallelism bought nothing (45.4s vs 46.8s) while peak memory went from **2.1 GB to 3.2 GB RSS**. That was enough to destabilise the Next server and intermittently lose one of the two trackers, which is exactly the "only one placement file shows" symptom. One at a time.
