...
**Two ways in: push for latency, schedule for completeness (`/api/ingest/placements/push`, `placement-mail-sync`).** Nothing sends placements to this application — it asks for them — so "as soon as a deal arrives" is a claim about how often we ask, and a schedule's honest ceiling is its interval. Polling every minute would not fix that; it would only be rude to an upstream that does a market-data lookup per ticker and an LLM call per summary on every read. `Placement_Email` already knows the exact moment: it classifies a broker mail, writes it to SQLite, and has the finished object in hand. One HTTP call from there is the entire difference between "within the hour" and "within a second".

- **The push does not replace the schedule.** A webhook that fails is silent by nature — the sender is not watching, and a deal pushed once and dropped is gone. `placement-mail-sync` (hourly across the Sydney trading day) is the backstop that finds it later. Note the route existed from the candidates migration with **no schedule at all** until this change; whatever ran it, ran by hand.
...
- **The Excel API edits in place; the file is never round-tripped.** The workbook is 13 MB and ~190 sheets of formulas, formatting and validation, and it is the desk's live book. Downloading it, editing with a spreadsheet library and uploading it back would rewrite all 190 sheets to change two, silently dropping every feature the library does not model. Graph adds a sheet, replays `Template` into it, patches five cells and appends one row; nothing else is touched.
- **Graph cannot copy a worksheet, so `Template` is REPLAYED.** `POST /worksheets/{id}/copy` answers `400 Resource not found for the segment 'copy'` on **v1.0 and beta**, and so does `range/copyFrom`; both were probed against the real workbook with `tables/add` as a control, which correctly returned 403 and proves the probe distinguishes a missing endpoint from a missing permission. A tab is therefore `worksheets/add` plus Template's own used range (`A1:P30`, read at write time rather than hardcoded) written to the same addresses, where its sheet-relative formulas stay correct.
- **Formatting does not survive that.** Fills, borders and column widths stay behind, including the yellow highlighting the desk's "ONLY EDIT FIELDS HIGHLIGHTED IN YELLOW" convention depends on — `range/cellProperties` does not exist here either, so there is nothing to replay from, and guessing which cells are meant to be yellow would mark the wrong ones editable. A new tab computes correctly and looks unstyled until someone formats it.
