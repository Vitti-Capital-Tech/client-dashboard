...
- **Fills and fonts ARE recovered, by treating a range read as a uniformity test (`tracker-style.ts`).** There is no bulk per-cell format read, but Graph answers a format read over a *range* with the value where every cell agrees and `null` where they do not — so the template's formatting can be read exactly by asking about the whole sheet, halving whatever comes back null, and asking again. Uniform blocks (the black header band, the yellow input cells) are answered whole; only the boundaries cost a split. Nothing here guesses which cells ought to be yellow. Borders are still not carried — eight separately addressed edges per range, for the part of the look nobody navigates by — and neither are validation or conditional formatting, which have no range-level read at all.
