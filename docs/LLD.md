...
Identity is a verified token, not a user-writable cookie. Sign-in is a **one-time code emailed to the address; there is no password** — see §8.32 for why, and for how the workspace is derived from the email domain.
...
`createUser` is called with `email_confirm: true` and **no password and no role**. Both omissions are deliberate: a password would be a second, weaker way in that no screen shows and nobody rotates (and `signInWithPassword` stays callable against the API whether or not a form for it exists), while the role belongs to the `auth.users` trigger (§8.32) — passing it here would only describe the rule twice, and the trigger would overwrite it anyway.
...
Sign-in is a **6-digit code emailed to the address**, and there is no password. The login screen already *had* a 6-digit OTP step; it collected the digits, threw them away, and called `signInWithPassword`. Its own comment said so — *"the OTP screen above is a cosmetic 2FA placeholder"*.

- **A code on top of a password is not a second factor.** `signInWithPassword` returns a fully valid session the moment the password is accepted, so anything after it is decoration a caller skips by using the token from step one. Supabase also has no email MFA factor — `mfa.*` is TOTP and phone — so email can only ever be a *primary* credential. Making the code the only credential removes the gap instead of papering over it: Supabase itself mints no session until `verifyOtp` succeeds, so there is no half-authenticated state for the proxy or RLS to be taught about.
...
- **Removing the password form does not disable password auth.** This Supabase version has no per-provider toggle for it, and the API keeps accepting `grant_type=password`. What actually closes it: no account holds a usable password (`createUser` is called without one, so `encrypted_password` is empty and cannot match), the one legacy demo password was rotated to a discarded random value, and signups are off so nobody can mint a new password-bearing account. Verified by asking the token endpoint for the old password and reading back `invalid_credentials`.
...
- **One login screen.** The `?role=` split is gone from `/login` and the landing page's two role buttons are one "Sign in": the role follows from the domain and is settled after the code is verified, so asking somebody to categorise themselves before they had proved who they were only offered a stranger two doors and a chance to guess wrong.
