...
- **P&L summary export (`lib/export/order-history.ts`):** one row per company — `Row Labels` (ticker), `Company`, `Buy Price`, `Sell Price / Current Price`, `PnL`, `Open Positions`, `Type` — plus a **Grand Total** row summing the three money columns. Honours the account filter, so the file always matches the screen.
