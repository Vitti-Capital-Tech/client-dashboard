...
- **`Convert to Options` — re-filing misbooked contract notes (`reclassifyTradesAsOptionAction`).** The broker sometimes books plainly-option transactions against the *ordinary* code, so `FRS` carries a sell side with no buys behind it and reads as a mismatch forever. It is not one: the trades belong on their own option line, which the P&L already reports and which this page skips entirely. The action rewrites the ledger rows — **`raw_security` is the field that matters**, because the engine reads the ticker from there and nowhere else (`dbTradesToParsedRows`), so touching `security_code` alone would change what the UI lists and leave every figure identical — plus `parent_code` (the 3-char underlying, keeping the option line beside its ordinary) and `instrument` (replacing the broker's `ORDINARY FULLY PAID` wording). Two guards, both about not moving settled notes onto another company: the new code must READ as an option (`isOptionCode` — past three characters with an `O` in the suffix, so `FRSX` is refused), and its parent must be the **same** underlying (`FRS → FRSO` yes, `FRS → ABCO` no). Audited by count and by both codes, then `recomputeClient` runs before it returns. The input pre-fills `<parent>O` but stays editable, because a company with several series in issue uses `FRSOA`/`FRSOB` and only the desk knows which.
- **`Mark Open` — the desk saying "this is held, not broken" (`markPositionOpenAction`).** Offered on any row where `buyQty > sellQty` **and the engine has not already said so**: `isDbOpenValued` means the holdings snapshot matched the parcel and valued it, which is the same statement this button makes and a better-sourced one — offering it there invited the desk to overwrite a snapshot figure with one typed at cost. Those rows stay listed if their quantities disagree; there is simply nothing left to declare. With **zero** sells the label drops "Excess Buy" entirely and reads `6,667 Bought, 0 Sold`, because nothing was sold for a buy side to be in excess *of* — it is an open parcel the holdings snapshot has not caught up with, typically a placement allocation not yet in the portfolio table. The engine already handles the case it can see (`mergeDbHoldings` sets both legs from the held quantity and marks the row `isDbOpenValued`); this is the same statement about a parcel the snapshot is silent on, so it is a recorded override with a note rather than an automatic rule. It writes **`sellQty = buyQty` and `sellOrCurrent = buyPrice`** — carried at cost. The second half matters as much as the first: left at zero the row reports a total loss of its whole cost base (DY6 read −$2,000.10 on a parcel that had lost nothing), which is a fabricated figure, not a conservative one.
...
- **Both paths share `storeCandidates`**, and that is the load-bearing part. If the push stored candidates by its own rules, the same deal could be fresh to one path and known to the other, and the tracker would grow a second tab for one placement. Covered by a test that pushes a deal and then runs the sync over it.
...
- **Fills, borders and fonts still do not survive a replay.** There is no bulk per-cell format read to replay them from, and guessing which cells are meant to be yellow would mark the wrong ones editable — worse than plain. A replayed tab computes and reads correctly but stays unshaded. The run's notes say so once, not once per deal, so "why has the new tab no yellow on it?" has a written answer.
...
- **Two duplicate guards, because the cron re-reads the same dates.** Only fresh candidates are attempted, and the Overview is re-checked by ticker **and** issue date before each write — a repeat placement in the same stock is legitimate and gets the desk's own `(b)` suffix, which is why the ticker alone cannot be the key.
...
- **Requires `Files.ReadWrite.All`** (plus `Sites.ReadWrite.All` for SharePoint) with admin consent. The app registration currently holds `Files.Read.All`, `Sites.Read.All` and `Mail.Read`, so the write path is inert and reports the exact consent step rather than failing obscurely.
