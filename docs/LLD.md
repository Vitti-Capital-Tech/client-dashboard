...
- **A sector chart with one bar labelled "Other"** is what `securities.sector` being null for most broker-feed tickers produces, and it reads as a broken chart rather than as missing data. It now says which it is. Labels also stopped being cut at the first word, where "Health Care" and "Health Insurance" both rendered as "Health".
