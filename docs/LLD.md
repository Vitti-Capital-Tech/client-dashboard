...
**Scheduling (`vercel.json`).** 22:00, 23:00 and 00:30 UTC on weekdays. Cron is UTC and Sydney is UTC+10 (AEST) / UTC+11 (AEDT), so a single fixed time is an hour wrong for half the year; rather than track the changeover, the job is made cheap to repeat — attachment dedupe plus idempotent importers mean a run with no new mail does almost nothing.
