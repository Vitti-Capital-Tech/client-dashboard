...
- **`realized_pnl`** — derived rollup at `(account_id, parent_code)`: units bought/sold/open, `cost_total`, `proceeds`, `cost_of_sold`, `open_cost`, `realized_pl`, `fees`, date range, plus two honesty flags (`has_partial`, `short_history`). Never hand-edited; `import-trades.mjs` owns it.
...
**CLIs (`scripts/`).** `_import-common.mjs` holds the `service_role` client, argv parsing, chunked upserts and error reporting.
- `import-holdings.mjs` — securities are upserted in **two passes** (all rows parent-less, then the links) because `parent_code` is a self-FK. Then clients → accounts → a **full replace** of `positions` scoped to the accounts in the file: anything the broker no longer reports has been sold and must disappear. Accounts absent from the file are untouched. Market Price lands in `securities.last_price` — currently the platform's only price source, so valuations are as stale as the last import.
- `import-trades.mjs` — creates stubs for ledger-only codes (a fully exited holding is absent from the snapshot, but `trades.security_code` is a real FK), upserts the notes, then **drops and rebuilds** `realized_pnl` by replaying each affected account's *whole stored ledger* rather than just this file's rows, so a partial export still yields correct cumulative figures. Finishes with the reconciliation and drift reports.
- Both accept `--dry-run`. `npm run import:holdings -- <file> [--dry-run]`, `npm run import:trades -- <file>`.
...
- **Tests:** `npm test` → `node --test "lib/**/*.test.ts" "store/**/*.test.ts"`, 162 tests, no framework and no dev-dependency. They cover day-first dates, parent codes, the reducer's flag semantics, reconciliation's refusal to guess, the CSV's grain separation and escaping, the P&L Calculator's partial-merge arithmetic, Black-Scholes (a textbook reference value plus the degenerate inputs that must collapse to intrinsic rather than `NaN`), the add-on spec parser against every shape the real workbooks contain (both column spellings, the `Unisted` typo, expiry-less cells dated off settlement, and a duplicated grant column that must not double the entitlement), and the calculator store's `useState`-compatible setters.
...
- **Data Models (`lib/pnl-calculator.ts`):**
  ...
  - `PlacementYearCandidate`: `{ issueYear, issueDate, totalShares, totalActualDollar, clientAllocations, addOns }` — one year's placement, kept apart from the other years' so they can never be added together, or have one year's option grants applied to another year's shares.
...
- **Placement Tracker Auto-Merge Engine (`mergePlacementTrackerIntoSummary`):**
  ...
  - Combines allocations across multiple Placement Tracker workbooks via **`combinePlacementMaps`** (in `lib/`, so it is testable).
    ...
    - Add-ons are taken from the **first** workbook that has them **within one year**, never concatenated — the same placement described twice would otherwise double every tranche. Across years they do NOT carry: see the ACM case below.
    - **Allocations are never summed across workbooks.** They were, and a ticker placed in both the 2025 and the 2026 tracker came out with both parcels stacked on one row — a Buy Qty and a cost base the client never had, and a P&L wrong by a whole placement. Either the same placement was carried forward (counting it twice is plainly wrong) or there really are two placements a year apart (real, but only one is what this trade file is about). Neither is summable, so each year becomes its own `PlacementYearCandidate` and the choice is deferred to the merge, which can see the trade dates.
    - Within ONE year the first workbook still wins per client, and a participant only the later workbook names is appended — the two files can carry different slices of the same participant list. Totals are recomputed off the unioned allocations so they cannot drift from their own rows.
    - A workbook whose year cannot be read is keyed by its **position** rather than lumped in with the others, so two undated files produce two candidates and get reported instead of one silently winning. The year comes from the Overview sheet's own date column, else the sheet name (`2025 Overview`), else any year in the workbook's sheet names, else the file name / URL passed as `parsePlacementTrackerBuffer(buffer, sourceName)`.
  - **Which year's placement belongs on a row (`chooseYearCandidate`).** One candidate is used as-is — the year check exists only to separate placements that would otherwise be stacked, so a single-year ticker still fills even from a ledger with no dates at all. With several candidates the ledger's **Contract Date** decides:
    - **BUY years first** — a placement is a purchase. A row with no recorded buys (the classic placement row: free or unnoted parcels never produce a contract note) falls back to every trade year on the row, since the sale is then the only date the ledger offers.
    - Exactly one candidate year matching wins. **Anything else fills nothing**: no trade dates at all (the ledger cannot answer), no year matching (tracker and trades disagree), or several matching (the client traded in both years, so picking one is a coin toss).
    - Trade years are collected in `aggregateTradesToSummary` onto `buyYears` / `tradeYears` (sorted, unique) via `parseTrackerDate`, which reads the Contract Date however the broker formatted it.
  - **Grants belong to a year, not to a ticker (`unlistedAddOnsFor`).** ACM was placed in June 2025 with `1:2@0.1 Unlisted` attached and again in January 2026 with an **empty** Add-Ons cell. The client took the 2026 parcel (ACMXX bought 04-02-2026), and because add-ons were held per TICKER — "the first workbook that has them" — the 2025 grant was applied to the 2026 shares and minted 23,333 options out of a placement the client was never in. `addOns` now live on the `PlacementYearCandidate`, and both `buildUnlistedOptionRows` and `collectUnlistedOptionTickers` read them through the SAME `chooseYearCandidate` the allocation merge uses, so an option row cannot come from a different placement than the shares that earned it. An unresolved year yields no grants either — the equity row is already blank and red, and an entitlement computed off an unknown parcel would be no better founded.
...
- **Unlisted placement options (`lib/black-scholes.ts` + `buildUnlistedOptionRows`):** free options attached to a placement, read from the Overview sheet's grant column — headed **Add-Ons** in the 2026 tracker and **Options** in the 2025 one — and valued with Black-Scholes because they do not trade.
...
- **Database Portfolio Holdings Sync (`mergeDbHoldingsIntoSummary`):**
  ...
  - **Orphan holdings get a row created for them.** The fill passes above can only annotate rows that already exist, and rows only exist for things that were *traded*. A **free placement option is never bought**, so no contract note exists and no row was ever built — which silently dropped the entire position from the P&L. (Real case: **106 of 108** option positions in the database carry `avg_cost = 0`; `GEDO` and `LITOC` are two. On account `1102011` this recovered **14 option rows worth $5,566.40** that previously vanished.) A second pass therefore creates a row per DB holding that no row matched:
    ...
    - Both legs are set from the held quantity, so the row reads as the open position it is. Flagged `isDbOnly` with `comment = "DB Holding"`, and `exportStatus()` returns `DB Holding` **ahead of** `Matched` — the legs reconcile only because they were set from the same number, so "Matched" would imply a trade reconciliation that never happened.
...
- **Interactive UI Capabilities (`PnlCalculatorClient.tsx`):**
  ...
  - **Comments** column between `PnL Calculated` and `Action`, carrying the derived note (`Open`, `DB Holding`, `Partial Buy`, `Partial Exit`, `Partial Buy · Partial Exit`, `Unlisted Options`). `DB Holding` stands alone rather than reading `Open · DB Holding`, since it already implies an open position valued off the snapshot. Present in the `.xlsx` and `.csv` exports as a trailing `Comments` column. `Open` is styled neutrally — it states a fact; the merge notes get amber.
  ...
  - Unlisted-option rows get a ⓘ button in the Ticker column opening a **hover card** with the full Black-Scholes breakdown (add-on text, entitlement, spot + source, strike, expiry, years, vol/rate/div, per-option price, row P&L) plus a warning when the spot was stale or missing. An expiry derived from the settlement date reads `… · assumed` and adds its own amber note naming the term, so nobody reads a convention as a figure off the tracker. Positioned `fixed` from the pointer and clamped to the viewport, because the results table sits in an `overflow-x-auto` wrapper and a scroll container clips on both axes.
