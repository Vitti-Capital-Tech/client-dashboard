...
- **Unlisted placement options (`lib/black-scholes.ts` + `buildUnlistedOptionRows`):** free options attached to a placement, read from the Overview sheet's **Add-Ons** column and valued with Black-Scholes because they do not trade.
  ...
  - Spot comes from `fetchSpotPricesAction()` — one batched `yahoo-finance2` v4 `quote()` call for `<TICKER>.AX`, falling back to `securities.last_price` from the last holdings snapshot. The `spotSource` (`yahoo` / `database` / `unavailable`) rides along and is shown in the UI; a name with no price is still booked (the entitlement is real) but valued at **$0** and reported in `skipped`, never defaulted to a strike or cost base.
...
- **Interactive UI Capabilities (`PnlCalculatorClient.tsx`):**
  ...
  - **Comments** column between `PnL Calculated` and `Action`, carrying the derived note (`Open`, `Partial Buy`, `Partial Exit`, `Partial Buy · Partial Exit`, `Unlisted Options`). Present in the `.xlsx` and `.csv` exports as a trailing `Comments` column. `Open` is styled neutrally — it states a fact; the merge notes get amber.
...
- **Session state (`store/usePnlCalculatorStore.ts`):** the calculator's working set is held in a **module-scope Zustand store**, not in the route component.
  ...
  - *Persisted:* `tradeFiles`, `result`, `placementFiles`, `parsedPlacementMap`, `selectedAccount`, `placementClient`, `placementUrl`, `filterType`, `searchQuery`.
