...
- **Grouping:** trades are grouped by parent ticker, each group headed by its realised result, ordered exactly as the chart orders its bars (realised descending; still-open companies last). Within a group, trades run **oldest first** — the buy that established the cost basis should be read before the sale that closed it.
- **Totals:** Bought / Sold / Brokerage+GST are summed client-side from the visible **settled** rows. Realised P&L is **not** `Sold − Bought` — most of what was bought is still held, so those two are not comparable; it comes from the replayed cost basis in `realized_pnl`.
- **Chart (`RealizedPnlChart.tsx`):** hand-written SVG, no charting dependency. A **diverging** horizontal bar chart, because the reader's job is polarity plus magnitude. Colour uses the design system's `--color-gain`/`--color-loss`; validated against the white surface that pair passes lightness, chroma and contrast but lands at **ΔE 7.2 under deuteranopia** — inside the 6–8 floor band, which is legal only alongside secondary encoding. Two are present and neither is decorative: **bar direction** (gains right, losses left of the baseline) and a **signed value label** on every bar. A reader who cannot separate the hues still reads the chart correctly. Bars are ≤ 24px with a 4px rounded data-end and a square baseline end; a non-zero result always gets a ≥ 2px stub, since sub-pixel bars read as a rendering fault rather than "very small".
- **Provisional figures:** bars whose parent has `short_history` are **hatched** and their label carries a caret. `EUR` is the longest bar in the sample account and is entirely unbacked — rendering it solid would be the most misleading thing the chart could do.
...
- **CSV export (`lib/export/order-history.ts`):** exports the visible groups, honouring the account filter. The file carries **two grains**, so a leading `Row type` column tags each row `SUMMARY` (per company: units sold, proceeds, cost of sold, realised P&L, fees, dates) or `TRADE` (per contract note). Repeating the realised figure on every trade row would make a careless `SUM` multiply it by the trade count; with the two grains in separate columns, summing either is safe. A `Cost basis` column carries each realised figure's caveat (`complete` / `approximate` / `MISSING - sold units never bought…`) so a provisional number never travels into a spreadsheet naked. RFC 4180 quoting plus a UTF-8 BOM for Excel.
- **Tests:** `npm test` → `node --test "lib/**/*.test.ts"`, 35 tests, no framework and no dev-dependency. They cover day-first dates, parent codes, the reducer's flag semantics, reconciliation's refusal to guess, and the CSV's grain separation and escaping.
