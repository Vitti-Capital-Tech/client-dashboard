...
- **Settlement Filtering & Ticker Aggregation:** Evaluates strictly `SETTLED` trades and automatically aggregates 5-character (or >3 char) derivative tickers (`EOSXX`, `ACWXX`) into 3-character parent codes (`EOS`, `ACW`). Tickers are sorted in **ascending alphabetical order** across UI tables and file exports.
...
- **Dynamic Exports:** Generates color-coded Excel (`.xlsx`) via ExcelJS server actions and CSV files in ascending ticker order for instant download.
