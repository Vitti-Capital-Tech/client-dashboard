...
- **Standing Placement Tracker links:** the tracker URL(s) can be configured permanently (`PLACEMENT_TRACKER_URL`, several links allowed) so the desk never pastes them again — the calculator loads them automatically the first time it is opened in a session. Read server-side only and never sent to the browser, because for a link-shared sheet the URL is the credential. Loaded once per session rather than per page view, since a real tracker is a ~12 MB workbook that takes ~12 s to parse.
...
- **Unlisted placement options (Black-Scholes):** the Placement Tracker's Overview **Add-Ons** column grants free options (`1:3 @$0.14 Unlisted Expiry 31/12/27`), sometimes several in one cell (a base tranche plus a `Piggyback` at a different strike and expiry). Each unlisted grant gets its own P&L row — quantity is `floor(basis × ratio)` where the basis is the **shares held** for a base tranche and the **base tranche's option count** for a piggyback (it is earned by exercising the base grant, not by holding stock), cost is zero, and value is a Black-Scholes call price with fixed desk assumptions (vol 50%, rate 5%, div 0%). Spot is sourced live from `yahoo-finance2`, then from the **ASX's own market-data feed** for anything Yahoo cannot quote, and only then from the last holdings snapshot — the source is carried through and shown, so a live price and a stale snapshot are never confused. Because they do not trade, the number is a **model estimate, not a mark**, and is labelled as such in the table, the hover card and the exports. Listed grants are skipped — they already arrive through the broker ledger.
