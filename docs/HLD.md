...
- **Reporting period:** an optional From/To window on the ledger's Contract Date. Left empty — the default — the calculator reports the lifetime P&L exactly as before; set, every figure covers only that period. **The window is applied in exactly one place, to the trades**, and everything else follows from that: the unlisted options appear for parcels *bought* in the period, because an entitlement runs off that Buy Qty. Filtering the placements by their own settlement date as well was tried and was wrong — a placement settles days before its shares are traded, so a window holding the trade missed the placement and the options disappeared from a period that plainly had them. Placement **allocations** are not filtered either: a parcel bought in 2025 and sold inside a 2026 window still needs its 2025 cost base, and hiding it would overstate the period's profit by that whole cost. The **holdings snapshot may not invent rows** while a period is set — it is "as of today" and has no date to place a position inside a window, which is how a stock the ledger never traded in the period turned up in it. Trades whose Contract Date cannot be read are left out and counted out loud, because a P&L that silently dropped rows is indistinguishable from one that had none. The period is named in the export filename for the same reason.
