...
- **Identifying WHICH YEAR's placement to use:** the same ticker can be placed in two different years, and both trackers are loaded at once. Adding the two parcels together — what the merge used to do — gave the client a Buy Qty and a cost base they never had, and a P&L wrong by a whole placement. The years are now kept apart and the ledger's **Contract Date** picks between them: a placement issued in 2025 is the one behind trades dated 2025, buy dates preferred over sell dates because a placement is a purchase. The **attached options follow the same year**, which matters as much as the shares: ACM was placed in 2025 with options attached and again in 2026 with none, and applying the 2025 grant to the 2026 parcel invented an option position the client never held. If the dates single out no year — none match, several match, or the ledger has no dates — **nothing is filled**: the row's Buy Qty, Buy Price and P&L show blank, the row is painted red with a `Check Placement Year` note, and it is left out of the totals rather than counted as a position bought for nothing. A guess here is indistinguishable from a real figure once it is in a spreadsheet; a red blank is not.
